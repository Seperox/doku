...
\ref{swidgenerator:architektur:printers} beschriebenen Optimierungen ist der
SWID Generator generell recht schnell. Nachfolgend ein paar
Geschwindigkeitsmessungen, die wir auf verschiedenen Systemen durchgeführt
haben. Getestet wurden dabei die folgenden Befehle:
...
sind, sind diese Resultate etwas schwer vergleichbar. Man kann jedoch die
ungefähr zu erwartende Laufzeit abschätzen.

Klar ersichtlich ist, dass der grösste Zeitaufwand beim Generieren von SWID Tags
\mit der texttt{-{}-full} Option geschieht. Dies ist verständlich, da hierbei
jede Datei in jedem Paket auf dem System vom SWID Generator auf Existenz geprüft
wird. An diesem Punkt gäbe es möglicherweise noch Optimierungspotential.

Auch das Pretty Printing ist ein \enquote{Zeitfresser}, da die generierten Tags
hierfür ein zweites mal geparsed und in einer \enquote{schöneren} Variante
ausgegeben werden müssen. Der Mehraufwand gegenüber der Variante ohne
\texttt{-{}-pretty} betrug in den drei gemessenen Beispielfällen zwischen 43\%
und 125\%. Da diese Option jedoch nur für Debugging-Zwecke gebraucht werden
sollte, ist dies in der Praxis kein nenneswertes Problem und auch kaum
ein sinnvolles Ziel für Optimierungen (auch wenn es da bestimmt noch Potential
gäbe).
