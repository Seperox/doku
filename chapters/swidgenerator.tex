...
\ref{swidgenerator:architektur:generators}.
...
Die Generatoren (im \texttt{generators} Package) sind dafür zuständig, um aus
den durch das entsprechende Environment zurückgegebenen Paketinformationen die
dazugehörigen Software IDs oder SWID Tags zu generieren.

Diese Funktionen sind als Python Generators\cite[S.~94-95]{alchin2010pro}
implementiert und geben die einzelnen Resultat-Strings mit \texttt{yield}
anstelle von \texttt{return} zurück. Dadurch kann man über alle SWID Tags oder
über alle Software-IDs iterieren, ohne dass alle Strings gleichzeitig in den
...
Die Generators werden nicht direkt evaluiert, sondern nach der Instanzierung an
die Printers weitergegeben. Mehr dazu im nächsten Abschnitt.
