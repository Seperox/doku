...
\begin{itemize}
	\item Ein Tag darf nur durch die Organisation modifiziert werden, die in
	...
	sich um den die Rolle des \enquote{Tag Creators} handeln. Das empfohlene
	Minimum besteht allerdings aus folgenden Informationen:
	\begin{itemize*}
...
Diese Anforderung ist bereits erfüllt, wenn nur der Name der Software und der
Name der Organisation die den Tag erstellt hat vorhanden sind.

In dieser Arbeit ist dieses Problem derzeit nicht schwerwiegend, da der SWID
Generator Tags generiert welche eindeutig Identifiziert werden können. Im
weiteren muss davon ausgegangen werden, dass es im Interesse des Tag Erstellers
liegt, dass seine SWID Tags nicht Ursache eines Konfliktes werden.
...
werden, sonder an Dateien in verschiedene Verzeichnisse kopieren.
...
und liefert diese direkt auf die Standardausgabe der Konsole. Die SIWD Tags
werden nie als Datei gespeichert.

\subsubsection{Doppelpunkte in der UniqueId} 
Der SWID Generator setzt die \texttt{uniqueId} aus dem Betriebsystem, der
Architektur, dem Paketnamen sowie der Versionsnummer zusammen:
