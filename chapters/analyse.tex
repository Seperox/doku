...
Bei der Umsetzung von Webapplikationen mit Django ist es gängig, dass das
...
Folgendes sind die zentralen Elemente der strongTNC Umgebung. Diese sind mit den
im folgenden verwendeten Namen als Django-Model implementiert. Diese Liste dient
dem Überblick, ausführlichere Beschreibungen sind in der Dokumentation
\enquote{Cygnet}\cite{cygnet:2013} zu finden.
\begin{description}
	\item[Policy] Eine Policy hat ein Typ der besagt welche Eigenschaft eines
	Gerätes überprüft werden soll, ausserdem werden abhängig vom Typ Details zur
	Messung angegeben. Zum Beispiel kann der Typ eine Überprüfung von offenen Ports
	darstellen, mit den Details, welche Ports geprüft werden sollen. Es wird
	weiterhin festgelegt welche Aktion bei positivem oder negativen Ausgang der
	Messung ausgeführt werden soll.
	
	\item[Enforcement] Policies werden durch Enforcements erzwungen. Das
	Enforcement wird einer Gruppe zugeteilt und bestimmt in welchen zeitlichen
	Abständen die verknüpfe Policy ausgeführt werden soll. Es können gegebenenfalls
	die Aktionen der Policy überschrieben werden.
	
	\item[Group] Verschiedene Elemente können Gruppen zugeteilt werden. Gruppen
	dienen letztendlich dazu, zu bestimmen welche Enforcements zur Anwendung
	kommen. Gruppen lassen sich hierarchisch aufbauen.
		
	\item[Session] Eine Session stellt den zeitlichen Rahmen einer Serie von
	Messungen dar. Wenn sich ein Gerät mit einem strongSwan VPN Netzwerk verbindet,
	wird durch strongSwan eine strongTNC Session für das Verbundene Gerät erstellt.
	
	\item[Workitem] Für jede Policy die durch ein Enforcement währen einer Session
	...
	Resultate werden nach den Messungen wieder in die Workitems zurück geschrieben.
	
	\item[Device] Ein Device repräsentiert ein Gerät, welches sich in ein
	strongSwan VPN Netzwerk verbinden kann. Ein Gerät wird durch die Kombination
	...
	Standardgruppen eines Gerätes mit einem bestimmten Betriebsystem dar.
	
	\item[Package] Ein Package ist ein Softwarepaket, es muss mindestens eine
	Version des Packages existieren, das heisst ein Version Element zugeordnet
	werden sein.
	
	\item[Version] Eine Version ist immer mit einem Package verknüpft. Die Version
	repräsentiert eine bestimmte Version des verknüpften Packages. Neben der
	Versionsbezeichnung muss auch ein Product für die Version erfasst sein. Eine
	Version kann \enquote{blacklisted} sein und so als Objekt einer Messung dienen.
	...
	\item[FileHash] Bei File-Messungen könne Hashes einer gewissen Datei erfasst
...
Wenn sich ein Gerät in ein strongSwan VPN Netzwerk verbindet, wird durch den OS
...
\begin{description}
	...
	kann eine Änderung im schlechtesten Fall eine Anpassung in allen Komponenten
	mit Datenbankzugriff zur Folge haben.
